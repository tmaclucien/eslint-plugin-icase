/**
 * @fileoverview locale key collection and generation
 * @author lucien
 * 此规则只在使用cli命令行npm run eslint生效, vscode插件下不生效
 */
"use strict";
const { splitMapIntoChunks } = require('../utils/helper.js');
const { defineTemplateBodyVisitor, isAlreadyWrapped} = require('../utils/ast-helper.js');
...
module.exports = {
  meta: {
    ...
    docs: {
      description: "This rule helps to collect locale keys and generate locale json file in vuejs project",
      recommended: true,
    ...
    const scriptVisitor = {
      Literal(node) {
        if (isAlreadyWrapped(node)) {
          const prefix = ruleConfig.prefix || 'icase'
           const hashKey = `${prefix}-${crc32(node.value).toString(16)}`
...
process.on('beforeExit', () => {
  ...
  const chunks = splitMapIntoChunks(collections) // chunk map array
  generate(chunks, ruleConfig)
  hasGenerated = true
});